        ...
        <>
          ...
          <div className="sm:hidden mb-3">
            <div className="flex items-center gap-2">
              ...
              >
                {versions.map((v) => (
                  <option key={v.id} value={v.id}>
                    {langFlag(v.language_code)} {langLabel(v.language_code)} ({v.sentences?.length ?? "…"})
                    {(v.progressToday ?? 0) > 0 ? " ·" + (v.progressToday === 100 ? " ✓" : " …") : ""}
                  </option>
                ))}
              </select>
              {/* Add language button — icon only on mobile */}
              {canEdit && (
                <button
                  onClick={() => setShowAddLang(true)}
                  className="flex-shrink-0 p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                  title={t("topics.addLanguage")}
                >
                  <PlusIcon className="w-4 h-4" />
                </button>
              )}
              {/* Version settings — icon only on mobile */}
              {canEdit && activeVersion && (
                <button
                  onClick={() => setVersionSettingsId(activeVersion.id)}
                  className="flex-shrink-0 p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              ...
              {/* Delete version — icon only on mobile */}
              {canEdit && activeVersion && versions.length > 0 && (
                <button
                  onClick={() => {
                    if (confirm(t("topics.deleteVersionConfirm", { lang: langLabel(activeVersion.language_code) }))) {
                      deleteVersionMutation.mutate(activeVersion.id);
                      setActiveVersionId(null);
                    }
                  }}
                  className="flex-shrink-0 p-2 rounded-lg border border-red-200 dark:border-red-800 text-red-400 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
          ...
          {/* ── Desktop: scrollable tab bar (≥ sm) ───────────────────── */}
          <div className="hidden sm:block relative -mx-4 sm:mx-0">
            {/* Right fade hint */}
            <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-10 bg-gradient-to-l from-gray-100 dark:from-gray-950 to-transparent z-10" />
              ...
              <div className="flex items-center border-b border-gray-200 dark:border-gray-700 min-w-max px-4 sm:px-0 snap-x snap-mandatory">
                    ...
                    <div key={v.id} className="group relative flex items-center">
                      {reorderMode && (
                        <div className="flex flex-col mr-0.5">
                          <button onClick={() => moveVersion(v.id, "up")} disabled={idx === 0}
                          ...
                          <button onClick={() => moveVersion(v.id, "down")} disabled={idx === versions.length - 1}
                            className="w-4 h-3.5 text-gray-400 hover:text-blue-500 disabled:opacity-20 disabled:cursor-not-allowed flex items-center justify-center"
                      ...
                      <button
                        ...
                        className={`relative flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors snap-start ${
                          isActive
                      ...
                      >
                        ...
                        {(v.progressToday ?? 0) > 0 && (
                          <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${v.progressToday === 100 ? "bg-green-500" : "bg-blue-400"}`}
                        ...
                        {canEdit && !reorderMode && (
                          <span onClick={(e) => { e.stopPropagation();
                            ...
                            title={t("topics.removeLanguageTitle")} role="button">
                            <XMarkIcon className="w-3.5 h-3.5" />
                          </span>
                        )}
                        {canEdit && !reorderMode && (
                          <span onClick={(e) => { e.stopPropagation(); setVersionSettingsId(v.id); }}
                            ...
                            title={t("topics.voiceSettingsTitle")} role="button">
                ...
                {canEdit && versions.length >= 2 && (
                  <button onClick={() => setReorderMode((v) => !v)}
                    className={`flex items-center gap-1 px-3 py-2.5 text-xs border-b-2 border-transparent transition-colors whitespace-nowrap ${
                      reorderMode ? "text-blue-600 dark:text-blue-400 font-semibold" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                    }`}
                ...
                <div className="flex items-center gap-2">
                  {hasRecordings && (
                    <Link
                      ...
                      params={{ topicId, langCode: activeVersion.language_code }}
                      className="inline-flex items-center gap-1 text-xs px-2 sm:px-3 py-1 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 font-medium transition-colors"
                      title={t("topics.reviewThis")}
                    >
                      <ClipboardDocumentListIcon className="w-3.5 h-3.5" />
                      <span className="hidden sm:inline">{t("topics.reviewThis")}</span>
                    </Link>
                  )}
                  <Link
                    to="/practice/$topicId/$langCode"
                    params={{ topicId, langCode: activeVersion.language_code }}
                    className="inline-flex items-center gap-1 text-xs px-2 sm:px-3 py-1 rounded-lg bg-green-600 hover:bg-green-700 text-white font-medium transition-colors"
                    title={t("topics.practiceThis")}
                  >
                    <PlayIcon className="w-3.5 h-3.5" />
                    <span className="hidden sm:inline">{t("topics.practiceThis")}</span>
                  </Link>
                </div>
              )}
            </div>
